...
const G={balloon:{n:'Balloon pop',max:25,rate:1},tap:{n:'Tap rush',max:60,rate:.5}};
const A={
...
gs(p,t){const u=U(t),g=G[p.g];if(!g)throw'Unknown game';if(u.pend)throw'Claim your pending reward first';if(u.d.pl>=C().plays)throw'No plays left today';u.d.pl++;u.g={t:Date.now(),id:p.g}},
ge(p,t){const u=U(t);if(!u.g)throw'Start a game first';wait(u.g.t,9.5);const g=G[u.g.id],n=Math.max(0,Math.min(+p.pops|0,g.max)),c=Math.floor(n*g.rate);u.g=0;if(c)u.pend={c,w:g.n};return{c}},
...
ac(p,t){const u=U(t);if(!u.a)throw'Start reading first';wait(u.a.t,C().artSecs);u.d.art.push(u.a.i);u.a=0;u.pend={c:C().artCoins,w:'Article'};return{c:C().artCoins}},
ads(p,t){const u=U(t);if(!u.pend&&u.d.ad>=C().adDaily)throw'Daily ad limit reached';u.ad=Date.now()},
adc(p,t){const u=U(t);wait(u.ad,C().adSecs);u.ad=0;let c,w;if(u.pend){c=u.pend.c;w=u.pend.w;u.pend=0}else{if(u.d.ad>=C().adDaily)throw'Daily ad limit reached';u.d.ad++;c=C().adCoins;w='Rewarded ad'}add(u,c,w);return{c}},
